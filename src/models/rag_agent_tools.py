...
get_file_by_source_func = FunctionDeclaration(
    ...
    description="Get the activity log chunks filtered by their data source",
    # Function parameters are specified in OpenAPI JSON schema format
    parameters={
        "type": "object",
        "properties": {
            "source": {"type": "string", "description": "The author name", "enum": ["Apple Health", "Strava", "Garmin Connect", "Whoop"]},
        ...
        "required": ["source", "search_content"],
    },
)


def get_file_by_source(data_source, search_content, collection, embed_func):
...
def execute_function_calls(function_calls, collection, embed_func):
    ...
    for function_call in function_calls:
        ...
        if function_call.name == "get_file_by_source":
            print("Calling function with args:",
                  function_call.args["source"], function_call.args["search_content"])
            response = get_file_by_source(
                function_call.args["source"], function_call.args["search_content"], collection, embed_func)
